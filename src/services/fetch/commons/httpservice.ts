 ...
 */
async function executeRequest<T>(requestFn: () => Promise<Response>, fallbackFn?: () => Promise<Response>): Promise<T> {
  try {
...
const http = {
  get: <T>(url: string, options: FetchOptions = {}): Promise<T> => {
    ...
    const requestWithoutProxy = options.useProxy !== false ? () => fetch(url, { ...options, method: 'GET' }) : undefined
  ...
  post: <T>(url: string, body: RequestBody = {}, options: FetchOptions = {}): Promise<T> => {
    ...
    const requestWithoutProxy = options.useProxy !== false ? () => fetch(url, finalOptions) : undefined
  ...
  delete: <T>(url: string, options: FetchOptions = {}): Promise<T> => {
    ...
    const requestWithoutProxy =
      options.useProxy !== false ? () => fetch(url, { ...options, method: 'DELETE' }) : undefined
...
export { http, getParams, proxyConfig, type FetchOptions, type RequestBody, type ProxyConfig }
