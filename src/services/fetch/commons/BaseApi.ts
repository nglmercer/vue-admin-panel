...
import { http, proxyConfig, type FetchOptions, type ProxyConfig } from './httpservice.ts'
...
const localStorage: Storage = typeof window !== 'undefined' ? window.localStorage || ssrSafeStorage : ssrSafeStorage
...
class BaseApi {
   ...
   */
  updateConfig(newConfig: { host?: string; port?: number | string; proxy?: ProxyConfig }): void {
   ...
   */
  async getWithoutProxy<T>(endpoint: string, options: Omit<FetchOptions, 'useProxy'> = {}): Promise<T> {
    return this.get<T>(endpoint, {
      ...options,
      useProxy: false,
    })
